...
{
    int unique_id_size;
    int user_size_length;
    int title_length, content_length;

    // contents에 "id:%d,major:%s,age:%d,introduction:%s,\n" 의 형식으로 문자열을 생성함
    sprintf(contents, "id:%d,user:%s,title:%s,content:%s,\n", 
        format->unique_id, format->user, format->title, format->content);
    printf("contents: %s\n", contents);
}
...
bool write_format_to_file(test_form *format)
{
    ...
    */
    int created_file_descriptor = file_open(
        "/home/eddi/proj/SDC-Study-Team1/hjtest18/created_file/format_test.txt", 
